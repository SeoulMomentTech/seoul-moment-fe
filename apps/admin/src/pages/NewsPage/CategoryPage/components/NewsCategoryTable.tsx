...
interface NewsCategoryTableProps {
  ...
  isLoading: boolean;
  isDeleting: boolean;
  onEdit(category: AdminNewsCategory): void;
  onDelete(categoryId: AdminNewsCategoryId): void;
}
        ...
        {categories.length === 0 ? (
          ...
          categories.map((category) => (
            <TableRow key={category.id}>
              ...
              <TableCell>{getCategoryName(category.nameList, "ko")}</TableCell>
              <TableCell>{getCategoryName(category.nameList, "en")}</TableCell>
              <TableCell>
                {getCategoryName(category.nameList, "zh-TW")}
              </TableCell>
              <TableCell>
                <Flex gap={8}>
                  <Button
