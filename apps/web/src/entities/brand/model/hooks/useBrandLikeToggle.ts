...
import { useCreateUserBrandLikeMutation } from "../../api/useCreateUserBrandLikeMutation";
import { useDeleteUserBrandLikeMutation } from "../../api/useDeleteUserBrandLikeMutation";

const LIKE_DEBOUNCE_MS = 400;

interface UseBrandLikeToggleProps {
  brandId: number;
  isLiked: boolean;
  likeCount: number;
}

interface UseBrandLikeToggleReturn {
  liked: boolean;
  count: number;
  handleToggleLike(): void;
}

export function useBrandLikeToggle({
  brandId,
  isLiked,
  likeCount,
}: UseBrandLikeToggleProps): UseBrandLikeToggleReturn {
